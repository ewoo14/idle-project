...
Then 무기 행은 `+2 / 50`, 비용 `900`, 성공률 `91%`를 표시한다.
...
Given 플레이어가 투구 슬롯에 +50 장비를 장착한다.

When HUD가 강화 패널을 렌더링한다.

Then 투구 행은 `+50 / 50` 레벨과 `MAX` 상태를 표시한다.

And 강화 비용과 성공률 칸도 `MAX`를 표시하고 강화 버튼은 클릭 대상에 등록되지 않는다.

Automation: `IdleProject.UI.HUD.EnhancePanelViewModel`

## 시나리오 3-1: 강화 HUD 로컬라이즈 키 정합

Given 한국어와 영어 UI CSV가 모두 로드된다.

When `ENHANCE_LEVEL_FORMAT`, `ENHANCE_SUCCESS_FORMAT`, `ENHANCE_STATUS_MAX` 키를 조회한다.

Then 영어는 `+N / 50`, `Success NN%`, `MAX`를 표시한다.

And 한국어는 같은 키를 누락 없이 제공하며 최대 상태를 현지화된 문구로 표시한다.

Automation: `IdleProject.Localization.CsvIntegrity`, `IdleProject.UI.HUD.EnhancePanelViewModel`
