...
## Scenario 6: server-client drop formula parity

Given the client formula uses `float` and the server mirror uses JavaScript
numbers.

When `server/src/core/formulas/drop.test.ts` runs the rarity multiplier, level
threshold, and slot bonus anchors.

Then `getRarityStatMultiplier`, `rollRarityForLevel`, and `computeItemBonus`
match the client `FDropFormula` boundaries, including `Math.fround` parity for
armor and accessory bonus splits.

And Lv1 Common variance `1.0` remains the legacy base bonus while Lv100 Rare,
Epic, and Legendary produce stronger generated stats before the existing #33
enhancement multiplier is applied.

Automation: `npm test -- src/core/formulas/drop.test.ts`,
