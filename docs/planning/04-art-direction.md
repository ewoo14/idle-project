...
## PR #67 Unique Trait HUD Addendum

This addendum belongs to UI/UX style section 3.2. Unique Trait HUD V1 extends
the existing left-bottom equipment summary instead of adding a new panel. Weapon
rows append trait lines directly below affixes, and armor/accessory rows append
compact `{Item}: {Trait} +{Value}` lines below the armor summary. Unique gear
shows one trait; Transcendent gear shows two traits in slot order.

Trait text reuses the existing rarity color and equipment HUD hierarchy, with no
new hard-coded colors. The Canvas-height scale clamp remains 1.0-2.0 so 1080p,
