...
Class Selection HUD V1은 좌측 상단 Canvas HUD 레이어에 `시작 직업` 패널로 배치하고 `전사`/`마법사`/`궁수` 3개 선택지를 고정 노출한다. 선택지는 `--bg-primary` 행 위에 현재 선택 직업만 `--accent-gold` 좌측 강조선과 `SELECTED` 상태를 사용하고, 비선택 행은 `--text-muted` 강조선과 `SELECT` 상태로 구분한다. 주 스탯 요약은 전사 `STR/CON`, 마법사 `INT`, 궁수 `DEX`로 표시하며, 1080p/1440p/4K에서는 Canvas 높이 기반 1.0~2.0 스케일로 좌측 28px 여백과 322px 기준 폭을 유지해 하단 스킬 HUD 및 우측 퀘스트/환생 HUD와 겹치지 않게 한다.
