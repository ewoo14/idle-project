...
const CLASS_GROWTH: Record<ClassId, ClassGrowth> = {
  1: {
    ...
    levelBonus: { str: 1.4, dex: 0.4, int: 0.15, wis: 0.15, con: 1.1, luk: 0.25 },
  },
  2: {
    initialBonus: { str: 2, dex: 3, int: 11, wis: 9, con: 4, luk: 3 },
    levelBonus: { str: 0.15, dex: 0.25, int: 1.4, wis: 1.1, con: 0.35, luk: 0.25 },
  },
  3: {
  ...
  4: {
    ...
    levelBonus: { str: 0.25, dex: 1.25, int: 0.2, wis: 0.2, con: 0.35, luk: 1.3 },
  },
  5: {
    initialBonus: { str: 2, dex: 3, int: 7, wis: 11, con: 5, luk: 4 },
    levelBonus: { str: 0.15, dex: 0.2, int: 1.0, wis: 1.4, con: 0.45, luk: 0.3 },
  },
};
...
export function defaultPrimaryStats(
  ...
  return {
    str: roundStat(sharedGrowth + profile.initialBonus.str + levelIndex * profile.levelBonus.str),
    dex: roundStat(sharedGrowth + profile.initialBonus.dex + levelIndex * profile.levelBonus.dex),
    int: roundStat(sharedGrowth + profile.initialBonus.int + levelIndex * profile.levelBonus.int),
    wis: roundStat(sharedGrowth + profile.initialBonus.wis + levelIndex * profile.levelBonus.wis),
    con: roundStat(sharedGrowth + profile.initialBonus.con + levelIndex * profile.levelBonus.con),
    luk: roundStat(sharedGrowth + profile.initialBonus.luk + levelIndex * profile.levelBonus.luk),
  };
}
...
export function deriveStats(
  ...
  return {
    ...
    mp: roundWhole(
      primary.wis * 5 + primary.int * 2 + level * 4 + 5 + bonus(equipmentBonus.mp),
    ),
    ...
    dodge: roundRate(
      clamp(
        primary.dex * 0.0015 + primary.luk * 0.001 + bonus(equipmentBonus.dodge),
        0,
        1,
      ),
    ),
