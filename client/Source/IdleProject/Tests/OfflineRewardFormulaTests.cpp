...
bool FOfflineRewardFormulaTest::RunTest(const FString& Parameters)
{
	...
	const FOfflineRewardResult UnderCap = FOfflineRewardFormula::ComputeOfflineRewards(
		10,
		1000,
		1000 + FOfflineRewardFormula::OFFLINE_CAP_SECONDS - 1,
		0);
	const FOfflineRewardResult OverCap = FOfflineRewardFormula::ComputeOfflineRewards(
		10,
		1000,
		1000 + FOfflineRewardFormula::OFFLINE_CAP_SECONDS + 1,
		0);
	TestEqual(TEXT("43199 seconds remains below cap"), UnderCap.CappedSeconds, static_cast<int64>(43199));
	TestEqual(TEXT("43201 seconds clamps to twelve hour cap"), OverCap.CappedSeconds, static_cast<int64>(43200));
	...
	const FOfflineRewardResult UnixEpoch = FOfflineRewardFormula::ComputeOfflineRewards(1, 0, 60, 0);
	TestEqual(TEXT("Unix epoch zero is a valid last seen timestamp"), UnixEpoch.CappedSeconds, static_cast<int64>(60));
	TestEqual(TEXT("Unix epoch zero gold mirrors server offline.ts"), UnixEpoch.Gold, static_cast<int64>(180));
	TestEqual(TEXT("Unix epoch zero exp mirrors server offline.ts"), UnixEpoch.Exp, static_cast<int64>(180));

	const FOfflineRewardResult OneSecond = FOfflineRewardFormula::ComputeOfflineRewards(1, 1000, 1001, 0);
	TestTrue(TEXT("Positive elapsed second gives non-zero gold"), OneSecond.Gold > 0);
	TestTrue(TEXT("Positive elapsed second gives non-zero exp"), OneSecond.Exp > 0);
	...
	TestEqual(
		TEXT("High-level gold base rate keeps int64 parity with server number"),
		FOfflineRewardFormula::BaseGoldPerSec(1000000000),
		static_cast<int64>(4000000000));
	TestEqual(
		TEXT("High-level exp base rate keeps int64 parity with server number"),
		FOfflineRewardFormula::BaseExpPerSec(1000000000),
		static_cast<int64>(4000000000));
