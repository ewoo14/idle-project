...
bool FIdleSaveGameDefaultsTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("SaveVersion starts at V26"), SaveGame->SaveVersion, static_cast<int32>(26));
...
bool FIdleSaveSystemApplyCaptureRoundTripTest::RunTest(const FString& Parameters)
{
	...
	SourceSave->PetStars.Add(TEXT("dog"), 3);
	SourceSave->PetStars.Add(TEXT("bird"), 1);
	...
	TestEqual(TEXT("Captured save writes V26"), CapturedSave->SaveVersion, static_cast<int32>(26));
	...
	TestEqual(TEXT("Dog star round trips"), CapturedSave->PetStars.FindRef(TEXT("dog")), static_cast<int32>(3));
	TestEqual(TEXT("Bird star round trips"), CapturedSave->PetStars.FindRef(TEXT("bird")), static_cast<int32>(1));
	...
	TestEqual(TEXT("Pet restore applies dog star"), PetService ? PetService->GetPetStar(TEXT("dog")) : INDEX_NONE, static_cast<int32>(3));
	TestEqual(TEXT("Pet restore applies bird star"), PetService ? PetService->GetPetStar(TEXT("bird")) : INDEX_NONE, static_cast<int32>(1));
...
bool FIdleSaveSystemLegacyV7StageMigrationTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Capture after legacy migration writes V26"), CapturedSave->SaveVersion, static_cast<int32>(26));
...
bool FIdleSaveSystemMasteryV12MigrationTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Migrated capture writes v26"), CapturedSave->SaveVersion, static_cast<int32>(26));
...
bool FIdleSaveSystemConsumableV13MigrationTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Migrated consumable capture writes v26"), CapturedSave->SaveVersion, static_cast<int32>(26));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FIdleSaveSystemConsumableGradeV15MigrationTest,
	"IdleProject.GameCore.SaveSystem.ConsumableGradeV15Migration",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FIdleSaveSystemConsumableGradeV15MigrationTest::RunTest(const FString& Parameters)
{
	// v15 엔트리는 Grade 필드가 없어 구조체 기본값 Standard(1) 가 그대로 남아야 합니다.
	UIdleSaveGame* LegacySave = NewObject<UIdleSaveGame>();
	LegacySave->SaveVersion = 15;
	LegacySave->bHasSave = true;
	FConsumableSaveEntry LegacyEntry;
	LegacyEntry.Type = static_cast<uint8>(EConsumableType::AttackTonic);
	LegacyEntry.Count = 3;
	LegacyEntry.BuffEndUnixSec = 0;
	TestEqual(TEXT("v15 entry defaults to Standard grade"), LegacyEntry.Grade, static_cast<uint8>(EConsumableGrade::Standard));
	LegacySave->Consumables.Add(LegacyEntry);

	UIdleGameInstance* GameInstance = NewObject<UIdleGameInstance>();
	TestTrue(TEXT("ApplyFromSave accepts v15 consumable save"), GameInstance->ApplyFromSave(LegacySave));
	UBuffService* BuffService = GameInstance->GetBuffService();
	TestNotNull(TEXT("Buff service exists after v15 migration"), BuffService);
	if (!BuffService)
	{
		return false;
	}

	TestEqual(TEXT("v15 stock migrates into Standard grade"), BuffService->GetCount(EConsumableType::AttackTonic, EConsumableGrade::Standard), 3);
	TestEqual(TEXT("v15 migration leaves Lesser grade empty"), BuffService->GetCount(EConsumableType::AttackTonic, EConsumableGrade::Lesser), 0);
	TestEqual(TEXT("v15 migration leaves Greater grade empty"), BuffService->GetCount(EConsumableType::AttackTonic, EConsumableGrade::Greater), 0);

	UIdleSaveGame* CapturedSave = NewObject<UIdleSaveGame>();
	TestTrue(TEXT("Capture after v15 grade migration succeeds"), GameInstance->CaptureToSave(CapturedSave));
	TestEqual(TEXT("v15 grade migration capture writes v26"), CapturedSave->SaveVersion, static_cast<int32>(26));
...
bool FIdleSaveSystemWeeklyBossV14MigrationTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Migrated weekly boss capture writes v26"), CapturedSave->SaveVersion, static_cast<int32>(26));
...
bool FIdleCloudSavePayloadMapperRoundTripTest::RunTest(const FString& Parameters)
{
	...
	ConsumableEntry.Grade = static_cast<uint8>(EConsumableGrade::Greater);
	...
	TestEqual(TEXT("Consumable grade survives cloud payload"), RestoredSave->Consumables.IsValidIndex(0) ? RestoredSave->Consumables[0].Grade : 255, static_cast<uint8>(EConsumableGrade::Greater));
